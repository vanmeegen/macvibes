...
function buildRunWrappers(spec: VmServicesSpec): Record<string, string> {
  ...
  return {
    ...
    'devserver-run.sh': [
      ...
      '# Subshell: daemon.env.sh liefert Egress-Proxy + Bun-Cache-Pfad, das',
      '# Proxy-Token darf aber NICHT an den devCommand (User-Code) leaken.',
      `( . ${VM_ETC_DIR}/daemon.env.sh 2>/dev/null; bun install --silent ) \\`,
      '  || echo "macvibes: Delta-Install fehlgeschlagen — devCommand startet trotzdem" >&2',
